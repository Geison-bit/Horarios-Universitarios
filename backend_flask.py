...
SUPABASE_KEY = os.environ.get("SUPABASE_KEY") # El nombre de la variable coincide
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY) # Y aquí también
...
def obtener_horarios_endpoint():
    ...
    try:
        ...
        print(f"Usuario verificado: {user.email} (ID: {user.id})")
    ...
    try:
        # La consulta ahora usa el user.id para filtrar
    ...
    for grupo_raw in grupos_data:
        ...
        if not all([nombre_curso_padre, tipo_componente_raw, nombre_grupo]): continue
        ...
        if nombre_curso_padre not in temp_cursos_struct: temp_cursos_struct[nombre_curso_padre] = {}
        if link_key not in temp_cursos_struct[nombre_curso_padre]: temp_cursos_struct[nombre_curso_padre][link_key] = {}
        if tipo_componente not in temp_cursos_struct[nombre_curso_padre][link_key]: temp_cursos_struct[nombre_curso_padre][link_key][tipo_componente] = []

        if not grupo_raw.get("sesiones"): continue
    ...
    filtro_func = None
    if filtro_param == "manana": filtro_func = solo_manana
    elif filtro_param == "tarde": filtro_func = solo_tarde
